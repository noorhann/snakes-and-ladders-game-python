...
# grid = [[i for i in reversed(range(91, 101))], list(range(81, 91)), [i for i in reversed(range(71, 81))],list(range(61, 71)), [i for i in reversed(range(51, 61))], list(range(41, 51)),[i for i in reversed(range(31, 41))], list(range(21, 31)), [i for i in reversed(range(11, 21))],list(range(1, 11))]

grid, player_position, snakes, ladders = [], [0, 0], [[], [], [], [], [], []], [[], [], [], [], [], []]
...
# This function prints the givAen dice face
...
def move_player(player, moves):
    player_position[player] = player_position[player] + moves
...
def generate_snakes_and_ladders(symbol):
    ...
    for i in range(n_components):
        ...
        while x1 == x2 or y1 == y2 or abs(x1 - x2) + abs(y1 - y2) <= int(min(N ** .5, M ** .5)):
            x1, y1 = random.randint(xb1, xe1), random.randint(yb1, ye1)
            x2, y2 = random.randint(xb2, xe2), random.randint(yb2, ye2)
        # grid[x1][y1] = symbol
        # grid[x2][y2] = symbol
        ...
        if symbol == ladder_symbol:
            ladders[i] = (min(p1, p2), max(p1, p2))
        if symbol == snake_symbol:
            snakes[i] = (min(p1, p2), max(p1, p2))
...
def grid_clear():
    grid.clear()
    grid.append([i for i in reversed(range(91, 101))])
    grid.append(list(range(81, 91)))
    grid.append([i for i in reversed(range(71, 81))])
    grid.append(list(range(61, 71)))
    grid.append([i for i in reversed(range(51, 61))])
    grid.append(list(range(41, 51)))
    grid.append([i for i in reversed(range(31, 41))])
    grid.append(list(range(21, 31)))
    grid.append([i for i in reversed(range(11, 21))])
    grid.append(list(range(1, 11)))
    player_position = [0, 0]
